...
class AniBD extends AnimeParser {
   ...
   */
  private parsePlayerConfig(source: string): PlayerConfig {
    ...
    const tracks: NonNullable<PlayerConfig['tracks']> = [];
  ...
  private extractStringProperty(source: string, property: string): string | null {
    const escapedProperty = property.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    const match = source.match(new RegExp(`\\b${escapedProperty}\\s*:\\s*(["'])((?:\\\\.|(?!\\1)[\\s\\S])*)\\1`));
    if (!match) return null;

    return match[2].replace(/\\(u[\da-fA-F]{4}|x[\da-fA-F]{2}|n|r|t|b|f|v|0|\\|"|')/g, (_full, escape: string) => {
